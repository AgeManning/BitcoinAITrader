...
def DBConnect():
    connection = pymysql.connect(host=Host, unix_socket=Socket, user=User, passwd=Pwd, db=DB)
    return connection
...
class BitCoinInterface:
    ...
    def __init__(self,Data):
        ## Data can be any of
        # (HomeCurrency, TradeCurrency)
        # (HomeCurrency, TradeCurrency, ExchangeDict)

        ## Intialise DB Variables from Globals. Can then be accesssed through the class
        self._User      = User
        self._Pwd       = Pwd
        self._DB        = DB
        self._Host      = Host
        ...
        if len(Data) == 2:
            ...
            self.UpdateExchangeDict() ## Get the Current Exchange Rates

        if len(Data) == 3:
            self._HomeCurrency    = Data[0]
            self._TradeCurrency   = Data[1]
            self._ExchangeDict    = Data[2]
    ...
    def GetInfo(self,Type):
        ...
        if Type == "Orders": # We have to loop through the orders
            for Order in Info['data']:
                Order['price']['value_int'] = self._ExchangeDict[self._HomeCurrency]/self.ExchangeDict[Order['currency']]*int(Order['price']['value_int'])
    ...
    def GetUseableCurrency(self):
        ...
        return (round(AvailableBTC,8), round(AvailableFunds,5))
    ...
    def GetLastTimeStamp(self):
        db = self.DBConnect()
        cursor = db.cursor()
        query = "SELECT MAX(UnixTimeStamp) FROM RawData;"
        cursor.execute(query)
